...
import {
  ...
  Plus,
  Trash2,
  FileImage, // Icon generik untuk file gambar
  FileCheck,
  Camera, // Icon opsional, cocok untuk HEIC (format kamera)
} from 'lucide-react';
...
export default function HeicToPdf() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Filter khusus HEIC/HEIF
      // Browser kadang tidak mendeteksi MIME type HEIC dengan benar, jadi kita cek ekstensi juga
      ...
      if (validFiles.length !== newFiles.length) {
        alert('Some files were skipped because they are not HEIC/HEIF images.');
      }
  ...
  const handleConvert = () => {
    ...
    // Catatan: Di produksi, Anda mungkin perlu library seperti 'heic2any' untuk client-side
    // atau mengirim file ke API server.
    setTimeout(() => {
      setIsProcessing(false);
      alert(`Successfully converted ${files.length} HEIC images to PDF!`);
    }, 2500); // Sedikit lebih lama dari BMP karena HEIC biasanya perlu decode
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-200 antialiased font-sans flex flex-col">
      ...
      <div className="flex-grow flex items-start justify-center py-12 p-4 bg-gray-900">
        <div className="bg-gray-800 rounded-xl shadow-xl px-4 py-8 md:p-8 max-w-2xl w-full text-gray-200 border border-gray-700">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">HEIC to PDF</h1>
          <p className="text-gray-400 mb-6">
            Convert one or more HEIC (High Efficiency) images from your iPhone
            or camera into a single PDF file.
          </p>
          ...
          >
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="text-xs text-gray-500">HEIC/HEIF images</p>
            ...
            <input
              ...
              className="hidden"
              multiple
              accept=".heic,.heif" // Spesifik HEIC
                  ...
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div className="bg-gray-800 p-2 rounded">
                        {/* Menggunakan Icon Camera karena HEIC adalah format foto kamera */}
