...
export default function WebPToPdf() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (webpFiles.length !== newFiles.length) {
        alert('Some files were skipped because they are not WebP images.');
      }
  ...
  const handleConvert = () => {
    ...
    setTimeout(() => {
      ...
      // Misalnya menggunakan library seperti jsPDF
      alert(
        `Successfully converted ${files.length} WebP files to PDF with ${quality} quality!`
      );
    }, 2000);
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-200 antialiased font-sans flex flex-col">
      ...
      <div className="flex-grow flex items-start justify-center py-12 p-4 bg-gray-900">
        <div className="bg-gray-800 rounded-xl shadow-xl px-4 py-8 md:p-8 max-w-2xl w-full text-gray-200 border border-gray-700">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">WebP to PDF</h1>
          ...
          >
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 text-sm text-gray-400">
                <span className="font-semibold text-gray-300">
                  Click to select WebP files
                </span>{' '}
                or drag and drop
              </p>
              <p className="text-xs text-gray-500">WebP Images</p>
            ...
            <input
              ...
              className="hidden"
              multiple
              accept="image/webp, .webp" // Spesifik WebP
