...
// import Image from 'next/image'; // Gunakan jika ingin optimasi gambar logo
import {
  ArrowLeft,
  UploadCloud,
  Instagram,
...
export default function PdfToJpg() {
  ...
  // State untuk kualitas gambar (0.1 - 1.0)
  const [quality, setQuality] = useState(0.9);

  // State simulasi file (untuk menampilkan panel opsi nanti)
  const [fileSelected, setFileSelected] = useState(false);

  // Handler saat file dipilih (Simulasi)
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      setFileSelected(true);
      // Nanti di sini logika simpan file ke state
    }
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-200 antialiased font-sans">
      ...
      >
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-white mb-2">PDF to JPG</h1>
          ...
          <div id="file-display-area" className="mt-4 space-y-2">
            {/* Area ini nanti diisi list file yang diupload */}
          </div>

          {/* Options Panel - Logic: Ditampilkan hanya jika file dipilih (atau bisa dihapus class hidden-nya untuk debug) */}
          <div
            id="options-panel"
            className={`mt-6 ${fileSelected ? 'block' : 'hidden'}`}
          >
            <div className="mb-4">
              <label
                htmlFor="jpg-quality"
